...
4. **Write the state file.** Create `~/.dev-diary/state.json` with:
   ```json
   { "diary_root": "<absolute path to the local working copy>" }
   ```
...
7. **Confirm hooks are active.** Since the plugin is installed, the `UserPromptSubmit` / `PostToolUse` / `Stop` hooks are already wired — no `~/.claude/settings.json` edits needed. Confirm the last `/reload-plugins` showed them loaded (non-zero hooks, no load error).

8. **Sanity check.** Suggest they send any small prompt that triggers a tool call, then end the session, and verify a `session-001.{yaml,md}` lands under `<diary_root>/entries/YYYY/MM/YYYY-MM-DD/` and gets committed + pushed.
